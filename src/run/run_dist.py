...
def run(_run, _config, _log):
    ...
    testing_algorithms = ["vdn", "qmix", "hpn_vdn", "hpn_qmix", "dpn_vdn", "dpn_qmix",
                          "deepset_vdn", "deepset_qmix", "deepset_hyper_vdn", "deepset_hyper_qmix",
                          "updet_vdn", "updet_qmix", "vdn_DA", "qmix_DA",
                          "gnn_vdn", "gnn_qmix", "qplex", "hpn_qplex", "asn", "casec", "casec_hpn"
                          ]
    env_name = args.env
    logdir = env_name
    if env_name in ["sc2"]:
...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        if buffer.can_sample(args.batch_size):
            ...
            episode_onsample = buffer.sample_latest(args.batch_size)
            
            # Truncate batch to only filled timesteps
            max_ep_t = episode_onsample.max_t_filled()
            episode_onsample = episode_onsample[:, :max_ep_t]

            if episode_onsample.device != args.device:
                episode_onsample.to(args.device)
            
            learner.train(episode_sample, episode_onsample, runner.t_env, episode)
            del episode_sample
