...
def build_distributional_td_lambda_targets(rewards, terminated, mask, target_qs, gamma, td_lambda):

    ret = target_qs.new_zeros(*target_qs.shape)
    ret[:, -1] = target_qs[:, -1] * (1 - th.sum(terminated, dim=1).unsqueeze(2)) 

    for t in range(ret.shape[1] - 2, -1, -1):
        ret[:, t] = td_lambda * gamma * ret[:, t + 1] + mask[:, t].unsqueeze(2) * (rewards[:, t].unsqueeze(2) + (1 - td_lambda) * gamma * target_qs[:, t + 1] * (1 - terminated[:, t]).unsqueeze(2))
