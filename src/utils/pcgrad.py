...
class PCGrad():
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None):
        ...
        for j in range(1, len(grads)):
            g_i_g_j = torch.dot(pc_grad[0], grads[j])

            # if g_i_g_j < 0:
            #     pc_grad[0] -= (g_i_g_j) * grads[j] / (grads[j].norm()**2 + 10e-8) # (grads[j].norm() * pc_grad[0].norm())
            # elif g_i_g_j > 0:
            #     pc_grad[0] += grads[j]
        ...
        merged_grad[shared] = pc_grad[0][shared]
        merged_grad[~shared] = pc_grad[0][~shared]
        return merged_grad
    '''
    def _project_conflicting(self, grads, has_grads, shapes=None):
        ...
        for g_i in pc_grad:
            ...
            for g_j in grads:
                ...
                if g_i_g_j < 0:
                    g_i -= (g_i_g_j) * g_j / (g_j.norm()**2 + 10e-8)
        merged_grad = torch.zeros_like(grads[0]).to(grads[0].device)
